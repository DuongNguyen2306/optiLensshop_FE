              ...
              {items.length > 0 ? (
                <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                  ...
                  <div className="space-y-2">
                    {items.map((item, i) => {
                      const vid = item.order_item_id ?? item.variant_id;
                      const name = typeof vid === "object" && vid
                        ? String((vid as Record<string, unknown>).name ?? (vid as Record<string, unknown>).sku ?? "—")
                      ...
                      return (
                        <div key={i} className="flex items-center justify-between rounded-lg border border-slate-100 bg-slate-50 px-3 py-2 text-sm">
                          ...
                          <span className="font-medium text-slate-900">× {String(item.quantity ?? 1)}</span>
                        </div>
                      );
                    })}
                  </div>
